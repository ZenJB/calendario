        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                                ...
                                @if(DB::table('cadeira_docente')->select('cadeira_id', 'docente_id')->where(['cadeira_id' => $cadeira->id, 'docente_id' => Auth::id()])->count() > 0)
                                    ...
                                    ->where([
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <form METHOD="post" action="{{route('api_delete_frequencia')}}">
                                                    {{csrf_field()}}
                ...
                <div class="card-small">
                    <div class="card-header">{{ __('Adicionar Frequencia') }}</div>
                    <table>
                        <thead>
                        <tr>
                            <th>Titulo</th>
                        ...
                        <tr>
                            <td>
                                <form id="criar_frequencia" method="POST" action="{{ route('api_criar_frequencia') }}">
                                    {{csrf_field()}}
                            ...
                            <td>
                                @if(isset($cadeiras))
                                <select form="criar_frequencia" name="cadeira">
                                        @foreach($cadeiras as $cadeira)
                                            @if(DB::table('cadeira_docente')->select('cadeira_id', 'docente_id')->where(['cadeira_id' => $cadeira->id, 'docente_id' => Auth::id()])->count() > 0)
                            ...
                            <td>
                                <input form="criar_frequencia"  type="text" name="descricao" value="">
                            </td>
